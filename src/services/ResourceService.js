...
async function checkAccess(currentUser, resources) {
  ...
  if (
    ...
    throw new errors.ForbiddenError(
      `Only M2M, admin or user with full access role can perform this action`
    );
  }
}
...
async function getResources(
  ...
  if (!challengeId && !memberId && !memberHandle) {
    throw new errors.BadRequestError(
      "At least one of the following parameters is required: [challengeId, memberId, memberHandle]"
    );
  }
  if (challengeId && !validateUUID(challengeId)) {
    throw new errors.BadRequestError(
      `Challenge ID ${challengeId} must be a valid v5 Challenge Id (UUID)`
    );
  }
  if (challengeId) {
    try {
        ...
        should: [
          { match_phrase: { memberId: currentUser.userId } },
          {
            bool: {
              ...
              must_not: [{ match_phrase: { memberId: currentUser.userId } }],
            },
          },
        ],
      },
  ...
  const resources = _.map(allResources, (item) => ({
    ...item,
    memberId: _.toString(item.memberId),
  }));
  ...
  for (const resource of resources) {
    const memberInfo = _.find(
      memberObjects,
      (o) => _.toNumber(o.userId) === _.toNumber(resource.memberId)
    );
    if (memberInfo) {
      const completeResource = {
        ...resource,
        rating:
          memberInfo && memberInfo.maxRating ? memberInfo.maxRating.rating : 0,
...
async function getResourceRole(roleId, isCreated) {
  try {
    ...
    if (error.name === "NotFoundError") {
      throw new errors.BadRequestError(
        `No resource role found with id: ${roleId}.`
      );
    } else {
      throw error;
    }
...
async function init(currentUser, challengeId, resource, isCreated) {
  // Verify that the challenge exists
  const challengeRes = await helper.getRequest(
    `${config.CHALLENGE_API_URL}/${challengeId}`
  );
  const challenge = challengeRes.body;

  if (
    _.get(challenge, "status") === constants.ChallengeStatuses.Completed &&
    !isCreated
  ) {
    throw new errors.BadRequestError(
      "Cannot delete resources of a completed challenge!"
    );
  }
  ...
  if (
    ...
    if (
      _.find(
        existing.data,
        (r) => r.roleId === config.SUBMITTER_RESOURCE_ROLE_ID
      )
    ) {
  ...
  if (
    ...
    const submissionsRes = await helper.getRequest(
      `${config.SUBMISSIONS_API_URL}`,
      { challengeId: challengeId, perPage: 100, memberId: memberId }
    );
    const submissions = submissionsRes.body;
    if (submissions.length !== 0) {
      throw new errors.ConflictError(
        `The member has already submitted to the challenge and cannot have a Reviewer or Iterative Reviewer role`
      );
    }
  }
  ...
  if (isCreated) {
    await helper.checkAgreedTerms(
      memberId,
      _.filter(
        _.get(challenge, "terms", []),
        (t) => t.roleId === resourceRole.id
      )
    );
  }
  if (!currentUser.isMachine && !helper.hasAdminRole(currentUser)) {
    ...
    if (!_.get(challenge, "legacy.selfService")) {
      if (
        ...
        _.toString(memberId) !== _.toString(currentUser.userId)
      ) {
  ...
  const dependencies = await ResourceRolePhaseDependencyService.getDependencies(
    { resourceRoleId: resource.roleId }
  );
  _.forEach(dependencies, (dependency) => {
    const phase = _.find(
      challenge.phases,
      (p) => p.phaseId === dependency.phaseId
    );
    if (phase) {
      ...
      if (_.isNil(isOpen)) {
        isOpen =
          ...
          phase.actualEndDate &&
          moment(phase.actualStartDate).utc() <= moment().utc() &&
          moment().utc() <= moment(phase.actualEndDate).utc();
      }
      if (_.isNil(isOpen)) {
        isOpen =
          phase.scheduledStartDate &&
          phase.scheduledEndDate &&
          moment(phase.scheduledStartDate).utc() <= moment().utc() &&
          moment().utc() <= moment(phase.scheduledEndDate).utc();
      }
      if (_.isNil(isOpen)) {
        isOpen = false;
      }
      if (!_.isEqual(isOpen, dependency.phaseState)) {
        throw new errors.BadRequestError(
          `Phase ${dependency.phaseId} should ${
...
async function createResource(currentUser, resource) {
  try {
    ...
    const { resources, memberId } = await init(
      currentUser,
      challengeId,
      resource,
      true
    ...
    if (
      _.reduce(
        resources,
        (result, r) =>
          _.toString(r.memberId) === _.toString(memberId) &&
          r.roleId === resource.roleId
            ? true
            : result,
        false
      )
      ...
      throw new errors.ConflictError(
        `User ${resource.memberHandle} already has resource with roleId: ${resource.roleId} in challenge: ${challengeId}`
      );
    }
...
async function deleteResource(currentUser, resource) {
  try {
    ...
    const { resources, memberId, handle } = await init(
      currentUser,
      challengeId,
      resource
    );
    ...
    if (!ret) {
      throw new errors.NotFoundError(
        ...
        } doesn't have resource with roleId: ${
          resource.roleId
...
async function listChallengesByMember(memberId, criteria) {
  ...
  if (criteria.resourceRoleId)
    boolQuery.push({ match_phrase: { roleId: criteria.resourceRoleId } });
